  ...
  use_density_(-1)
{
    ...
    while (iarg < narg && hasargs)
    {
        ...
        if (strcmp(arg[iarg],"type") == 0) {
            ...
            if (iarg+4 > narg)
                error->fix_error(FLERR,this,"not enough arguments for 'tflag'"); // FEG - changed fflag to tflag so the error message is more specific
            iarg++;
            if(strcmp(arg[iarg],"on") == 0)
                tflag_[0] = true;
            else if(strcmp(arg[iarg],"off") == 0)
                ...
                error->fix_error(FLERR,this,"expecting 'on or 'off' after 'tflag'"); // FEG - changed fflag to tflag so the error message is more specific
            iarg++;
            if(strcmp(arg[iarg],"on") == 0)
                tflag_[1] = true;
            else if(strcmp(arg[iarg],"off") == 0)
