...
void FixMultisphere::init() 
{
  ...
  if(modify->n_fixes_style("nonviscous") > 1) //FEG
    error->fix_error(FLERR,this,"only one fix nonviscous supported"); //FEG
...
void FixMultisphere::calc_force(bool setupflag)
{
  ...
  if(fix_nonviscous_) //FEG
  {
      ...
      {
            fcm[ibody][0] -= damp*fabs(fcm[ibody][0])*fix_nonviscous_->sign(vcm[ibody][0]); //FEG
            fcm[ibody][1] -= damp*fabs(fcm[ibody][1])*fix_nonviscous_->sign(vcm[ibody][1]); //FEG
            fcm[ibody][2] -= damp*fabs(fcm[ibody][2])*fix_nonviscous_->sign(vcm[ibody][2]); //FEG
      }
  }
