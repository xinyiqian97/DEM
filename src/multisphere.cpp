               ...
               double **displace_ins, bool *fflag, bool *tflag, int start_step_ins,double *v_integrate_ins)
{
    ...
    bool fflags[3] = {fflag[0],fflag[1],fflag[2]}; // FEG - fixed error
    bool tflags[3] = {tflag[0],tflag[1],tflag[2]}; // FEG - fixed error
    ...
    fflag_.set(n,fflags); //FEG - changed to fflags
    tflag_.set(n,tflags); //FEG - changed to tflags
