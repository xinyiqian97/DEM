  ...
  Compute(lmp, iarg, narg, arg)
{
  ...
  // do not store overlapRatio or squaredforce by default // things added by FEG
  overlapRatioflag = squaredforceflag = 0; //FEG
  ...
  if(narg > iarg)
     posflag = velflag = idflag = fflag = fnflag = ftflag = torqueflag = torquenflag = torquetflag = histflag = areaflag = deltaflag = heatflag = overlapRatioflag = squaredforceflag = cpflag  = 0; // FEG - added overlapRatioflag, squaredforceflag
  ...
  for (; iarg < narg; iarg++)
  {
    ...
    else if (strcmp(arg[iarg],"overlapRatio") == 0) overlapRatioflag = 1; // FEG
    else if (strcmp(arg[iarg],"squaredForce") == 0) squaredforceflag = 1; // FEG
...
void ComputePairGranLocal::init_cpgl(bool requestflag)
{
  ...
  nvalues = posflag*6 + velflag*6 + idflag*3 + fflag*3 + fnflag*3 + ftflag*3 + torqueflag*3 + torquenflag*3 + torquetflag*3 + histflag*dnum + areaflag + deltaflag + heatflag + overlapRatioflag + squaredforceflag + cpflag*3; //FEG - added overlapRatioflag, squaredforceflag
...
void ComputePairGranLocal::add_pair(int i,int j,double fx,double fy,double fz,double tor1,double tor2,double tor3,double *hist)
{
    ...
    if(overlapRatioflag) // FEG
    { // FEG
        radi = atom->radius[i]; // FEG
        radj = atom->radius[j]; // FEG
        vectorSubtract3D(atom->x[i],atom->x[j],del); // FEG
        array[ipair][n++] = 0.5*(radi+radj-vectorMag3D(del))/std::min(radi,radj); // FEG
        
    } // FEG
    if(squaredforceflag) // FEG
    { // FEG
        array[ipair][n++] = fx*fx+fy*fy+fz*fz; // FEG
    } // FEG
...
void ComputePairGranLocal::add_wall_2(int i,double fx,double fy,double fz,double tor1,double tor2,double tor3,double *hist,double rsq, double *normal)
{
    ...
    if(overlapRatioflag) // FEG
    { // FEG
        array[ipair][n++] = 0.5*(atom->radius[i]-sqrt(rsq)) / (atom->radius[i]); // FEG
    } // FEG
    if(squaredforceflag) // FEG
    { // FEG
        array[ipair][n++] = fx*fx+fy*fy+fz*fz; // FEG
